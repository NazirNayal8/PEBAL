...
class BDD100KSeg(Dataset):
    ...
    def _read_data_file(self, path):
        ...
        for f in fl:
            ...
            if ',' in p:
                p = p.split(',')
                lst_1.extend([os.path.join(self.hparams.dataset_root, p[0])])
                lst_2.extend([os.path.join(self.hparams.dataset_root, p[1])])
            else:
                lst_1.extend(lst_1.extend([os.path.join(self.hparams.dataset_root, p[0])]))
